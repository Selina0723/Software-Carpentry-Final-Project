...
class App():
    def __init__(self):
        ...
        self.image = self.canvas.create_image(
            0, 0, anchor='nw', image=self.imagefile)
    ...
    def mainWindows(self):
        ...
        self.entry_usr_name = tk.Entry(
            self.mainWindow, textvariable=var_usr_name)
        self.entry_usr_name.place(x=200, y=100)

        var_usr_pwd = tk.StringVar()
        self.entry_usr_pwd = tk.Entry(
            self.mainWindow, textvariable=var_usr_pwd, show='*')
        ...
        bt_logquit = tk.Button(
            self.mainWindow,
            text='QUIT',
            bg='skyblue',
            command=self.usr_sign_quit)
    ...
    def usr_log_in(self):
        ...
        if usr_name == '' or usr_pwd == '':
            ...
            is_signup = messagebox.askyesno(
                'Welcome!', message='You have not registered yet, do you want to register now?')
            if is_signup:
                self.usr_sign_up()
        else:
            for row in rows:
                if usr_name == row[1]:
                    if usr_pwd == row[2]:
                        messagebox.showinfo(
                            title='Welcome!', message='Welcome ' + usr_name)
    ...
    def usr_sign_up(self):
        def signtowcg():
            ...
            if np == '' or nn == '':
                tk.messagebox.showerror(
                    'ERROE', 'Username or password is empty!')
            elif np != npf:
                tk.messagebox.showerror(
                    'ERROR', 'The password is inconsistent!')

            else:
                insert_sql = """insert into userTable(username,password)values("{}", "{}");""".format(
                    nn, np)
        ...
        self.usernameEntry = tk.Entry(
            self.registerWindows, textvariable=new_name)
        self.usernameEntry.place(x=150, y=10)
        # Password
        new_pwd = tk.StringVar()
        tk.Label(
            self.registerWindows,
            text='Please enter your password:').place(
            x=10,
            y=50)
        ...
        tk.Label(
            self.registerWindows,
            text='Please enter your password again:').place(
            x=10,
            y=90)
        self.okPasswordEntry = tk.Entry(
            self.registerWindows,
...
class Function():
    ...
    def AddPatInfo(self):
        ...
        self.TypeOfDiseaseEntry = tk.Entry(
            self.addWindows, textvariable=self.TypeOfDiseaseVar)
        self.TypeOfDiseaseEntry.place(x=230, y=160, height=40, width=200)
        self.RecoveryTimeEntry = tk.Entry(
            self.addWindows, textvariable=self.RecoveryTimeVar)
        ...
        def OkInsert():
            ...
            try:
                ...
                messagebox.showerror(
                    "ERROR", message="Recovery Time must be a valid number!")
                return

            self.cursor.execute(
                "SELECT * FROM patInfoTable WHERE patId = ?", (patId,))
            num = self.cursor.fetchall()
            if num:
                messagebox.showwarning(
                    "WARNING", message="The patient ID already exists, please enter again!")
            else:
                try:
                    ...
                    messagebox.showinfo(
                        ...
                        message=f"{name}'s record has been successfully added!")
                except sqlite3.Error as e:
                    messagebox.showerror(
                        "ERROR", message=f"Failed to add record: {e}")
        ...
        # Exit
        DeleteButton = tk.Button(self.addWindows, text="Exit", font=(
            "Arial -20"), bg="blue", command=self.addWindows.destroy)
    ...
    def DelPatInfo(self):
        ...
        def OkDelete():
            ...
            if patId == "":
                messagebox.showerror(
                    "ERROR", message="Please Enter the Number!")
                return
            # Search
            self.cursor.execute(
                "select * from patInfoTable where patId = '%s';" %
                patId)
            num = self.cursor.fetchall()
            if num:
                self.cursor.execute(
                    "delete from patInfoTable where patId = '%s';" %
                    patId)
                self.conn.commit()
                messagebox.showinfo(
                    title="Congratulations!",
                ...
                messagebox.showwarning(
                    "WARNING",
                    message="The patient's information does not exist, please enter it first!")
        ...
        # Exit
        DeleteButton2 = tk.Button(self.delWindows, text="Exit", font=(
            "Arial -20"), bg="blue", command=self.delWindows.destroy)
    ...
    def changePatInfo(self):
        ...
        # Label
        changePatIdLabel = tk.Label(
            self.changeWindows,
            text="Number:",
            font=("Arial -20"))
        changePatIdLabel.place(x=130, y=10, height=40, width=100)
        self.changePatIdEntry = tk.Entry(
            self.changeWindows, textvariable=self.changePatIdVar)
        self.changePatIdEntry.place(x=250, y=10, height=40, width=200)

        def OkSelect():
            if self.changePatIdEntry.get() == "":
                messagebox.showerror(
                    title="ERROR", message="Please Enter the Number!")
                return
            ...
            if l_row:
                ...
                messagebox.showwarning(
                    title="WARNING",
                    message="The patient information does not exist, please enter first!")
            ...
            self.nameEntry = tk.Entry(
                self.changeWindows, textvariable=self.nameVar)
            self.nameEntry.place(x=320, y=210, height=40, width=180)
            self.TypeOfDiseaseEntry = tk.Entry(
                self.changeWindows, textvariable=self.TypeOfDiseaseVar)
            self.TypeOfDiseaseEntry.place(x=320, y=260, height=40, width=180)
            self.RecoveryTimeEntry = tk.Entry(
                self.changeWindows, textvariable=self.RecoveryTimeVar)
            ...
            def saveInfo():
                ...
                if name == "" or TypeOfDisease == "" or self.RecoveryTimeEntry.get() == "":
                    messagebox.showerror(
                        "ERROR", message="The message cannot be empty!")
                    return
                RecoveryTime = eval(self. RecoveryTimeEntry.get())
                if RecoveryTime < 0 or RecoveryTime > 100000000:
                    messagebox.showerror(
                        "ERROR", message="Patient information entered incorrectly!!!")
                else:
        ...
        DeleteButton = tk.Button(
            ...
            font="Arial -20",
            bg="blue",
            command=self.changeWindows.destroy)
    ...
    def selectPatInfo(self):
        ...
        self.selectWindows_frame1 = tk.Frame(
            self.selectWindows, width=600, height=100)
        self.selectWindows_frame1.place(x=0, y=0)
        self.selectWindows_frame2 = tk.Frame(
            self.selectWindows, width=600, height=500)
        self.selectWindows_frame2.place(x=0, y=100)

        self.selectPatIdVar = tk.StringVar(self.selectWindows_frame1, value='')

        selectPatIdLabel = tk.Label(
            self.selectWindows_frame1,
            text="Number:",
            font=("Arial -20"))
        selectPatIdLabel.place(x=10, y=30, height=40, width=80)
        self.selectPatIdEntry = tk.Entry(
            self.selectWindows_frame1,
            textvariable=self.selectPatIdVar)
        ...
        def OkSelect():
            ...
            if self.patId == "":
                messagebox.showerror(
                    "ERROR", message="The patient number cannot be empty！")
                return
            ...
            if l_row:
                for row in l_row:
                    ...
                    if self.patId == row[1]:
                        ...
                        tree = tkinter.ttk.Treeview(
                            self.selectWindows_frame2, columns=(
                                'c1', 'c2', 'c3', 'c4'), show='headings', height=500)
                        ...
                        # Treeview
                        tree.place(
                            relx=0.0, rely=0.0, relwidth=1.0, relheight=1.0)
                        ls_data = [
                            [self.patId, name, TypeOfDisease, RecoveryTime]]
                        for data in ls_data:
                ...
                messagebox.showwarning(
                    "WARNING",
                    message="The patient information does not exist, please enter it first!")
        # search all
        ...
        def OkSelectAll():
            ...
            scrollbar = tkinter.Scrollbar(
                self.selectWindows_frame2, bg="blue", command=tree.yview)
            ...
            try:
                ...
                for data in all_data[1:]:
                    ...
                    try:
                        ...
                        sql1 += "VALUES('%s','%s','%s',%f)" % (patId,
                                                               name, TypeOfDisease, RecoveryTime)
                        self.cursor.execute(sql1)
                        self.conn.commit()
                    except BaseException:
                        messagebox.showerror(
                            "WARNING",
                            message="The format of patient results information is wrong and cannot be entered!")
                messagebox.showwarning(
                    title="WARNING",
                    message="If the patient number is repeated, it cannot be entered!!")
                messagebox.showinfo(
                    title="Congratulations!",
                    message="Patient information batch entry success!")
            except Exception as e:
                messagebox.showerror(
                    "ERROR", message=f"The form format is wrong, please re-select!!:{e}")

    def exportExcel(self):
        file_path = filedialog.asksaveasfilename(
            title="Export File", initialfile="pat_data.xlsx", filetypes=[
                ("Excel Workbook", "*.xlsx")])
        if file_path == "":
            ...
            messagebox.showinfo(
                title="Congratulations",
                message="Patient information exported successfully!")
    ...
    def taskManagement(self):
        ...
        self.task_listbox = tk.Listbox(
            self.taskWindows, height=10, selectmode="single")
        ...
        def save_tasks():
            file_path = filedialog.asksaveasfilename(
                title="Save Tasks", defaultextension=".json", filetypes=[
                    ("JSON files", "*.json")])
            if file_path:
        ...
        tk.Button(
            self.taskWindows,
            text="Exit",
            command=self.taskWindows.destroy).place(
            x=170,
            y=320,
            width=150,
            height=25)
    ...
    def taskProgress(self):
        ...
        tk.Button(
            self.progressWindows,
            text="Exit",
            command=self.progressWindows.destroy).place(
            x=200,
            y=380,
            width=100,
            height=25)
